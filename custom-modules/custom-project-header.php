...
class Custom_ET_Builder_Module_Fullwidth_Project_Header extends ET_Builder_Module {
	function init() {
		$this->name             = esc_html__( 'Custom HYPE Project Header', 'et_builder' );
		$this->slug             = 'et_pb_fullwidth_project_header';
		$this->vb_support       = 'on';
		$this->fullwidth        = true;
		...
		$this->advanced_fields = array(
			...
			'background' => array(
				'css'     => array(
					'main' => '.et_pb_fullwidth_project_header%%order_class%%',
				),
				'options' => array(
	...
	function render( $attrs, $content = null, $render_slug ) {
		...
		$output = sprintf(
			'	%6$s
				%8$s
				<div class="et_pb_fullwidth_project_header_container%5$s">
					%2$s
					%3$s
				</div>
...
new Custom_ET_Builder_Module_Fullwidth_Project_Header;
