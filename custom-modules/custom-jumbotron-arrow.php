...
class Custom_ET_Builder_Module_Fullwidth_Header extends ET_Builder_Module {
	function init() {
		$this->name             = esc_html__( 'Custom Fullwidth HYPE Jumbotron', 'et_builder' );
		$this->slug             = 'et_pb_fullwidth_header_2';
		$this->vb_support       = 'on';
		$this->fullwidth        = true;
	...
	function render( $attrs, $content = null, $render_slug ) {
		...
		if ( 'off' !== $header_scroll_down || '' !== $scroll_down_icon ) {
			$scroll_down_output .= sprintf(
				'<a href="#"><span class="scroll-down et-pb-icon bounce-effect">%1$s</span></a>',
...
new Custom_ET_Builder_Module_Fullwidth_Header;